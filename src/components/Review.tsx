...
function Review(props: any) {
    return (
        <li key="1" className="flex space-x-4 mb-4 rounded dark:shadow justify-between">
            ...
            <div>
                <div className="flex"><span><Link to ={`/${props.user}/${props.series && props.series.replace(/\s/g, '-')}/${props.reviewIndex}/`}>Review by</Link></span>&nbsp;<Link to={`/${props.user}`}>{props.user}</Link>&nbsp;
